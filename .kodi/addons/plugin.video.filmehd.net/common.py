...
def ROart(f,fe=''): return xbmc.translatePath(os.path.join(_artPath,'ro',f+fe)) ### for Making path+filename+ext data for Art Images. ###
...
def ParseDescription(plot): ## Cleans up the dumb number stuff thats ugly.
	...
	if not addst('my-language','Romanian').lower()=='romanian': 
	#if addst('my-language','Romanian').lower()=='romanian': 
...
def messupText(t,_html=False,_ende=False,_a=False,Slashes=False):
	...
	#t=t.replace("text:u","")
	return t
def nURL(url,method='get',form_data={},headers={},html='',proxy='',User_Agent='',cookie_file='',load_cookie=False,save_cookie=False,ReturnObj=False):
	if url=='': return ''
	dhtml=''+html
	...
	#debob(['Cookies',net.get_cookies()])
	#debob(['Cookies*',net._cj._cookies])
	if ReturnObj==True: return html,net
	return html
...
def PlayURLs(url):
	...
	if (uPre.lower()=='mss') or (uPre.lower()=='mssh') or (uPre.lower()=='rtsp'):
		...
		importURLResolver(); #import urlresolver
		try:
...
def ContextMenu_Series(labs={},TyP='tv'):
	...
	try:
		...
		LB['subfav']='1'; 
		if fav__COMMON__check(LB['site'],LB['section'],LB['title'],LB['year'],LB['subfav'])==True: LB['mode']=McFR; LabelName=WRFCr+WRFC+'Filme - Movies'; #addst('fav.tv.'+LB['subfav']+'.name'); 
