...
def search(params):
    ...
    for match in matches:
        ...
        title = re.sub('\s+', ' ', title).strip() + "  (" + get_language_from_flag_img(flag) + "  " + rating + "  " + quality + ")"
...
def movies_updates(params):
    ...
    for match in matches:
        ...
        if not (only_english and get_language_from_flag_img(flag) != "EN"):
            ...
            url = urljoin(params.get("url"),scrapedurl)
            title = html_unescape(scrapedtitle).strip()
            title=title + "  (" + get_language_from_flag_img(flag) + "  " + rating + "  " + quality + ")"
...
def single_movie(params):
    ...
    thumbnail = re.sub('^https', 'http', thumbnail) # otherwise: CCurlFile::Stat - Failed: SSL connect error(35) for https: ...
    ...
    for hoster_url, hoster_name, quality, date_added, hoster_thumbnail in hosters:
        title = hoster_name + "  " + quality + "  (Added " + date_added + ")"
...
def resolve_media_direct_link(hosted_media_url):
    ...
    plugintools.log("movie4k.play hosted_media_file=" + repr(bool(hosted_media_file)) + " " + repr(hosted_media_file))
    if hosted_media_file:
        direct_link = hosted_media_file.resolve()
        plugintools.log("movie4k.play media_url=" + repr(bool(direct_link)) + " " + repr(direct_link))
        if direct_link:
